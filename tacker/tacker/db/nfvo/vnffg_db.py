...
class VnffgPluginDbMixin(vnffg.VNFFGPluginBase, db_base.CommonDbMixin):
    ...
    def _process_parameterized_template(self, dev_attrs, vnffgd_template):
        ...
        if isinstance(param_vattrs_dict, dict):
            self._update_template_params(
                vnffgd_template, param_vattrs_dict, param_matched)
        else:
    ...
    def get_vnffg_by_vnf_id(self, context, vnf_id):
        ...
        for vnffg_info in vnffgs:
            ...
            with context.session.begin(subtransactions=True):
                ...
                if vnffg.get('attributes') and \
                    ...
                    vnffgd_topology_template = \
                        template_db.template['vnffgd']['topology_template']
                    self._process_parameterized_template(
                        vnffg_param, vnffgd_topology_template)
                    template_db.template['vnffgd']['topology_template'] = \
                        vnffgd_topology_template
    ...
    def _get_resource_ports(self, context, vnf_id, vnffg):
        ...
        """
        vnffg_id = vnffg['id']
        vnffgnfp_db = (self._model_query(context, VnffgNfp).filter(
            VnffgNfp.vnffg_id == vnffg_id).one())
        ...
        logical_chain = self._get_nfp_attribute(template_db.template,
                                                nfp_name, 'path')
        cp_items = dict()
        for index, element in enumerate(logical_chain):
            vnf_resources = vnfm_plugin.get_vnf_resources(context, vnf_id)
            for resource in vnf_resources:
                cp_name = resource['name']
                LOG.debug('cp name %s', cp_name)
                if cp_name == element['capability']:
                    cp_items['name'] = resource['name']
                    cp_items['id'] = resource['id']
                    cp_items['ppg_index'] = index
        return cp_items

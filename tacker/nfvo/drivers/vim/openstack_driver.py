...
class OpenStack_Driver(abstract_vim_driver.VimAbstractDriver,
    ...
    # Update vnffg corresponding to the policy type of vnf: scaling in/out
    def update_vnffg_polices(self, port_chain_id, resource_port,
                             vnf_info, policy_type, auth_attr=None):
        if not auth_attr:
            LOG.warning("auth information required for n-sfc driver")
            return None
        neutronclient_ = NeutronClient(auth_attr)
        ...
        try:
            desired_ppg_id = \
                port_chain['port_chain'][
                    'port_pair_groups'][resource_port['ppg_index']]
        except Exception:
            LOG.error('port pair group not found')
            raise nfvo.PPGNotFoundException(pc_id=port_chain_id)
        desired_ppg_info = neutronclient_.port_pair_group_show(desired_ppg_id)
        ppg_update = dict()
        ppg_update['name'] = desired_ppg_info['port_pair_group']['name']
        ppg_update['description'] =\
            desired_ppg_info['port_pair_group']['description']
        ppg_update['port_pairs'] = []

        if policy_type == constants.HEALING_EVT:
            port_pair = dict()
            # Update port pair
            port_pair['name'] =\
                vnf_info['name'] + '-RESPAWN-' + 'connection-points'
            port_pair['description'] =\
                'port pair for %s' % resource_port['name']
            port_pair['ingress'] = resource_port['id']
            port_pair['egress'] = resource_port['id']
            port_pair_id = neutronclient_.port_pair_create(port_pair)
            ppg_update['port_pairs'].append(port_pair_id)

            ppg_new = neutronclient_.port_pair_group_update(desired_ppg_id,
                                                            ppg_update)
        return ppg_new

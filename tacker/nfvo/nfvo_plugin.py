...
class NfvoPlugin(nfvo_db_plugin.NfvoPluginDb, vnffg_db.VnffgPluginDbMixin,
    ...
    @log.log
    def trigger_healing_function(self, context, vnf_id):
        vnffg_list = self.get_vnffg_by_vnf_id(context, vnf_id)
        # TODO(anyone): In this case, vnf just belongs to one vnffg
        vnfm_plugin = manager.TackerManager.get_service_plugins()['VNFM']
        for vnffg_id in vnffg_list:
            vnffg_db = super(NfvoPlugin, self).get_vnffg(context, vnffg_id)
            # Find VNF resources
            vnf_info = vnfm_plugin.get_vnf_resources(context, vnf_id)
            resource_ports = self._get_resource_ports(context, vnf_id=vnf_id,
                                                    vnffg=vnffg_db)
            vim_obj = self._get_vim_from_vnf(context, vnf_id)
            port_chain_id = self.get_port_chain_id(context, vnffg_id)
            driver_type = vim_obj['type']
            ppg_update = self._vim_drivers.invoke(
                driver_type, 'update_vnffg_policies',
                port_chain_id=port_chain_id,
                resource_ports=resource_ports,
                vnf_info=vnf_info,
                policy_type=constants.HEALING_EVT,
                auth_attr=vim_obj['auth_cred'])
        return ppg_update

    @log.log
    def trigger_scaling_function(self, context, vnf_id):
        vnffg_list = self.get_vnffg_by_vnf_id(context, vnf_id)
        # TODO(anyone): In this case, vnf just belongs to one vnffg
        for vnffg_id in vnffg_list:
            vnffg_db = super(NfvoPlugin, self).get_vnffg(context, vnffg_id)
            ...
            ppg_update = self._vim_drivers.invoke(
                driver_type, 'update_vnffg_policies',
                port_chain_id=port_chain_id,
                scaling_ports=resource_ports,
                policy_type=constants.SCALING_OUT_EVT,
                auth_attr=vim_obj['auth_cred'])
        return ppg_update

        ...
        def _execute_action(action):
            ...
            try:
                if action in constants.DEFAULT_ALARM_ACTIONS:
                    ...
                    status = _establish_rpc(target, 'vnf_respawning_event')
                    LOG.debug('RPC respawning output: %s', status)
                    if status == constants.ACTIVE:
                        # TODO(anyone) must use proper method to emit events
                        nfvo_plugin =\
                            manager.TackerManager.get_service_plugins()['NFVO']
                        try:
                            result = nfvo_plugin.trigger_healing_function(context, vnf_id)
                        except Exception as ex:
                            LOG.debug('Fail to get default vim due to %s', ex)
                            raise vnfm.VNFEventFailed(
                                event_name=constants.SCALING_EVT, vnf_id=vnf_id)

                else:
                    target = AutoScalingRPC.AutoScalingRPC.target
                    status = _establish_rpc(target, 'vnf_scaling_event')
                    LOG.debug('RPC scaling output: %s', status)
                    if status == constants.ACTIVE:
                        # TODO(anyone) must use proper method to emit events
                        nfvo_plugin = \
                            manager.TackerManager.get_service_plugins()['NFVO']
                        try:
                            ...
                            LOG.debug('Fail to get default vim due to %s', ex)
                            raise vnfm.VNFEventFailed(
                                event_name=constants.HEALING_EVT, vnf_id=vnf_id)
